...
export default function BlogPage() {
  ...
  // Sample blog posts with images and slugs
  const blogPosts = [
    {
      id: 1,
  ...
  return (
    <div className="min-h-screen bg-white">
      ...
      {/* Blog Header */}
      <section className="bg-[#F5F5DC] text-[#6B8E23] py-16">
        <div className="container mx-auto px-4 text-center">
          <h1 className="text-4xl lg:text-5xl font-bold mb-4">Vegist Blog</h1>
          <p className="text-xl lg:text-2xl font-semibold text-[#6B8E23] max-w-3xl mx-auto">
            Discover recipes, cooking tips, health benefits, and the latest news from Vegist
          </p>
        </div>
      </section>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-4 gap-8">
              ...
              <div className="bg-white rounded-lg shadow-sm p-6 border border-gray-200">
                ...
                <div className="relative">
                  <input
                    ...
                    className="w-full border border-gray-300 rounded-lg px-4 py-3 pr-10 focus:ring-2 focus:ring-[#6B8E23] focus:border-[#6B8E23] focus:outline-none"
                  />
                  <svg
              ...
              <div className="bg-white rounded-lg shadow-sm p-6 border border-gray-200">
                ...
                <div className="space-y-4">
                  {recentPosts.map(post => (
                    <div key={post.slug} className="pb-4 border-b border-gray-100 last:border-0 last:pb-0">
                      <Link
                        to={`/blog/${post.slug}`}
            ...
            <div className="lg:col-span-3">
              <div className="flex justify-between items-center mb-8">
                ...
                <span className="text-gray-600 bg-[#6B8E23] text-white px-3 py-1 rounded-full text-sm">
              ...
              {filteredPosts.length > 0 ? (
                <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
                  {filteredPosts.map(post => (
                    <article key={post.id} className="bg-white rounded-lg shadow-sm overflow-hidden border border-gray-200 hover:shadow-md transition-all duration-300">
                      <Link to={`/blog/${post.slug}`} className="block">
                        ...
                        <div className="p-5">
                          {/* Category Tag */}
                          <div className="mb-3">
                            <span className="inline-block bg-[#6B8E23] text-white text-xs px-2 py-1 rounded-full capitalize">
                              {post.category}
                            </span>
                          </div>
                          
                          {/* Title */}
                          <h3 className="text-lg font-bold text-gray-800 mb-3 line-clamp-2 hover:text-[#6B8E23] transition-colors">
                            {post.title}
                          </h3>
                          ...
                          {/* Footer with Read More and Metadata */}
                          <div className="flex justify-between items-center pt-3 border-t border-gray-100">
                            <Link 
                              to={`/blog/${post.slug}`}
                              className="text-[#6B8E23] hover:text-[#5A7A1A] font-medium text-sm flex items-center group"
                            >
                              Read more 
                              <svg 
                ...
                <div className="text-center py-12 bg-white rounded-lg shadow-sm border border-gray-200">
                  <div className="text-6xl mb-4 text-[#6B8E23]">📝</div>
                  ...
                  <button
                    ...
                    className="bg-[#6B8E23] hover:bg-[#5A7A1A] text-white font-medium py-2 px-6 rounded-lg transition-colors"
                  >
                    Reset Filters
                  </button>
                </div>
              ...
              {filteredPosts.length > 0 && filteredPosts.length >= 9 && (
                <div className="text-center mt-12">
                  <button className="bg-[#6B8E23] hover:bg-[#5A7A1A] text-white font-medium py-3 px-8 rounded-lg transition-colors shadow-sm hover:shadow-md">
                    Load More Articles
                  </button>
                </div>
              )}
